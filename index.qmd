...
```{r}
GDP_CHINA <- read.csv("C:/Users/prisc/OneDrive - Calvin University/2Spring/Stat341/Currency-Exchange-Rate-Project/datafiles/CHNLORSGPNOSTSAM.csv")
GDP_CHINA <- GDP_CHINA |>
...
*All variables except Inflation Rate are measured monthly, and I really would like  monthly data as exchange rate can change dramatically within a year, so I still need to figure out what to do with Inflation rate data that are NA right now. *
...
*I also think the later exchange rate would be influenced by the previous exchange rate, but I don't know how to include that in the model. Does Bayesian approach automatically that this into acount?*
